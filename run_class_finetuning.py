...
from datasets import build_dataset, build_fold_dataset
...
def main(args, ds_init):
    ...
    train_iter = build_fold_dataset(is_train=True, test_mode=False, args=args)
    val_iter = build_fold_dataset(is_train=False, test_mode=False, args=args)
    test_iter = build_fold_dataset(is_train=False, test_mode=True, args=args)
    ...
    for fold, (dataset_train, args.nb_classes) in enumerate(train_iter):
        ...
        # val_subset = torch.utils.data.Subset(dataset_val, val_idx)
        ...
        # num_training_steps_per_epoch = len(train_subset) // total_batch_size
        ...
        # print("Number of training examples = %d" % len(train_subset))
        ...
        print("Max WD = %.7f, Min WD = %.7f" % (max(wd_schedule_values, default=0), min(wd_schedule_values, default=0)))
            ...
            if args.output_dir and args.save_ckpt:
                if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                    utils.save_model(
                        ...
                        loss_scaler=loss_scaler, epoch=epoch, model_ema=model_ema, fold=fold+1)
            if data_loader_val is not None:
                ...
                if max_accuracy < test_stats["acc1"]:
                    ...
                    if args.output_dir and args.save_ckpt and not args.no_save_best_ckpt:
                        utils.save_model(
                            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                            loss_scaler=loss_scaler, epoch="best", model_ema=model_ema, fold=fold+1)
